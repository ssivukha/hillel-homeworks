...
class CocktailsList {
    ...
    render() {
        ...
        this.filteredList.forEach(function (item) {
            ...
            orderButton.appendChild(document.createTextNode('buy'));
...
class Order {
    ...
    renderOrder() {
        ...
        this.orderList.forEach(function (item) {
            ...
            cocktailItem.innerText = `<div>${item.name}</div> + <div>${item.name}</div> + <div>${item.price}</div> + <div>${this.totalPrice()}</div>`;
